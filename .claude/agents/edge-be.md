...
- **Security:** wrap payload logging in `sanitizeForDevLog` (`shared/log-sanitize.ts`). Never log Mysoft credentials, tokens, or OTP codes. Secrets live in the root `.env` only and reach Deno via `[edge_runtime.secrets]` in `supabase/config.toml` — add new keys there.
