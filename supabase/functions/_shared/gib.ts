...
import {
  fetchInvoicePreviewWithClient,
  type InvoicePreviewContent,
} from "./invoice-preview.ts";

export { mapInvoicesToFacts } from "./invoice-mapper.ts";
export type { InvoiceFactRow, InvoiceLineItem } from "./invoice-mapper.ts";
export { GibAuthError };
export type { CreateInvoiceInput, GibAuthErrorCode, GibAuthTraceEntry };
...
const USER_DATA_PATCH_KEYS: (keyof UserData)[] = [
  "taxIDOrTRID",
  "title",
  "name",
  "surname",
...
/** GİB `updateUserData` tam nesne ister; mevcut profille birleştirir. */
export function mergeGibUserDataPatch(
  current: UserData,
  patch: Record<string, unknown>,
): UserData {
  const next: UserData = { ...current };
  for (const key of USER_DATA_PATCH_KEYS) {
...
/** Araç / HTTP gövdesinden sadece izinli string alanları ayıklar. */
export function extractGibUserDataStringPatch(
...
async function persistSessionToken(
  ...
  const { error } = await supabaseAdmin.from("gib_sessions").upsert(
    {
      gib_username: key,
      token: null,
...
export async function gibCreateInvoicePreview(
  ...
  return withSession(username, async (client, token) => {
    ...
    const draft = await client.createDraftInvoice(token, details);

    // Taslak listeye düşmeden HTML isteği bazen GİB'de patlıyor; önce findInvoice bekle.
    ...
    let html = "";
    try {
      const preview = await fetchInvoicePreviewWithClient(
        client,
        token,
        draft.uuid,
        false,
      ...
      console.warn(
        JSON.stringify({
          event: "gib_preview_fetch_failed",
          uuid: draft.uuid,
...
export async function gibGetInvoiceHtml(
  ...
  throw new Error("GİB HTML önizlemesi alınamadı.");
}

export type { InvoicePreviewContent } from "./invoice-preview.ts";

export async function gibGetInvoicePreview(
  username: string,
  uuid: string,
  signed: boolean,
  draftDate?: string,
  ...
  return withSession(username, (client, token) =>
    fetchInvoicePreviewWithClient(client, token, uuid, signed, draftDate),
  );
}
...
export async function gibCancelInvoice(
  ...
  const todayFormatted = todayGibApiFormatted();
  return withSession(username, async (client, token) => {
...
export async function gibGetUserData(username: string): Promise<UserData> {
  return withSession(
    username,
    (client, token) => client.getUserData(token) as Promise<UserData>,
  );
}
